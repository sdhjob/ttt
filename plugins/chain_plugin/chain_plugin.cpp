...
#include <sou/chain/fork_database.hpp>
#include <sou/chain/block_log.hpp>
#include <sou/chain/exceptions.hpp>
#include <sou/chain/authorization_manager.hpp>
#include <sou/chain/code_object.hpp>
#include <sou/chain/config.hpp>
#include <sou/chain/wasm_interface.hpp>
#include <sou/chain/resource_limits.hpp>
#include <sou/chain/reversible_block_object.hpp>
#include <sou/chain/controller.hpp>
#include <sou/chain/generated_transaction_object.hpp>
#include <sou/chain/global_property_object.hpp>
#include <sou/chain/snapshot.hpp>

#include <sou/chain/sou_contract.hpp>
...
std::ostream& operator<<(std::ostream& osm, sou::chain::db_read_mode m) {
   if ( m == sou::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == sou::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == sou::chain::db_read_mode::READ_ONLY ) {
      osm << "read-only";
   } else if ( m == sou::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              sou::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(sou::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(sou::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(sou::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(sou::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, sou::chain::validation_mode m) {
   if ( m == sou::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == sou::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              sou::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(sou::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(sou::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace sou::chain::config;
using namespace sou::chain::plugin_interface;
...
:my(new chain_plugin_impl()) {
   app().register_config_type<sou::chain::db_read_mode>();
   app().register_config_type<sou::chain::validation_mode>();
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<sou::chain::wasm_interface::vm_type>()->value_name("runtime")->notifier([](const auto& vm){
#ifndef SOU_VM_OC_DEVELOPER
            //throwing an exception here (like SOU_ASSERT) is just gobbled up with a "Failed to initialize" error :(
            if(vm == wasm_interface::vm_type::sou_vm_oc) {
               elog("SOU VM OC is a tier-up compiler and works in conjunction with the configured base WASM runtime. Enable SOU VM OC via 'sou-vm-oc-enable' option");
               SOU_ASSERT(false, plugin_exception, "");
            }
#endif
         ...
         ("read-mode", boost::program_options::value<sou::chain::db_read_mode>()->default_value(sou::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<sou::chain::validation_mode>()->default_value(sou::chain::validation_mode::FULL),
...
#ifdef SOU_VM_OC_RUNTIME_ENABLED
         ("sou-vm-oc-cache-size-mb", bpo::value<uint64_t>()->default_value(souvmoc::config().cache_size / (1024u*1024u)), "Maximum size (in MiB) of the SOU VM OC code cache")
         ("sou-vm-oc-compile-threads", bpo::value<uint64_t>()->default_value(1u)->notifier([](const auto t) {
               if(t == 0) {
                  elog("sou-vm-oc-compile-threads must be set to a non-zero value");
                  SOU_ASSERT(false, plugin_exception, "");
               }
         }), "Number of threads to use for SOU VM OC tier-up")
         ("sou-vm-oc-enable", bpo::bool_switch(), "Enable SOU VM OC tier-up runtime")
#endif
         ;
...
if( options.count(op_name) ) { \
   ...
   for( const auto& v : ops ) { \
      container.emplace( sou::chain::name( v ) ); \
   } \
}
...
protocol_feature_set initialize_protocol_features( const fc::path& p, bool populate_missing_builtins = true ) {
   ...
   if( fc::exists( p ) ) {
      SOU_ASSERT( fc::is_directory( p ), plugin_exception,
   ...
   if( directory_exists ) {
      for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
         ...
         SOU_ASSERT( res.second, plugin_exception,
   ...
   auto output_protocol_feature = [&p]( const builtin_protocol_feature& f, const digest_type& feature_digest ) {
      ...
      SOU_ASSERT( !fc::exists( file_path ), plugin_exception,
   ...
   ( builtin_protocol_feature_t codename ) -> digest_type {
      ...
      if( !res.second ) {
         SOU_ASSERT( res.first->second, protocol_feature_exception,
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if SOU_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "SOU_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::sou_root_key));
         throw;
      }
      ...
      if( options.at( "print-build-info" ).as<bool>() || options.count( "extract-build-info") ) {
         ...
         if( options.count( "extract-build-info") ) {
            ...
            SOU_ASSERT( fc::json::save_to_file( chainbase::environment(), p, true ), misc_exception,
         ...
         SOU_THROW( node_management_success, "reported build environment information" );
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            SOU_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               SOU_ASSERT( itr->second == item.second,
      ...
      if( options.count( "chain-threads" )) {
         ...
         SOU_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      SOU_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         if( options.count( "extract-genesis-json" )) {
            ...
            SOU_ASSERT( fc::json::save_to_file( *gs, p, true ),
                        misc_exception,
         ...
         SOU_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if( options.count("export-reversible-blocks") ) {
         ...
         SOU_THROW( node_management_success, "exported reversible blocks" );
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         SOU_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
         ...
         SOU_THROW( node_management_success, "imported reversible blocks" );
      }
      ...
      if (options.count( "snapshot" )) {
         ...
         SOU_ASSERT( fc::exists(*my->snapshot_path), plugin_config_exception,
         ...
         SOU_ASSERT( !fc::is_regular_file(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            if( block_log_genesis ) {
               ...
               SOU_ASSERT( *chain_id == block_log_chain_id,
                           plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" ) ) {
            ...
            if( chain_id ) {
               SOU_ASSERT( *block_log_chain_id == *chain_id, block_log_exception,
         ...
         if( options.count( "genesis-json" ) ) {
            ...
            SOU_ASSERT( fc::is_regular_file( genesis_file ),
                        plugin_config_exception,
            ...
            if( block_log_genesis ) {
               SOU_ASSERT( *block_log_genesis == provided_genesis, plugin_config_exception,
               ...
               if( chain_id ) {
                  SOU_ASSERT( provided_genesis_chain_id == *chain_id, plugin_config_exception,
                  ...
                  if( block_log_chain_id ) {
                     SOU_ASSERT( provided_genesis_chain_id == *block_log_chain_id, plugin_config_exception,
                                 "Genesis state, provided via command line arguments, has a chain ID (${provided_genesis_chain_id}) "
         ...
         if( !chain_id ) {
            if( my->genesis ) {
               ...
               SOU_ASSERT( !block_log_chain_id, plugin_config_exception,
...
#ifdef SOU_VM_OC_RUNTIME_ENABLED
      if( options.count("sou-vm-oc-cache-size-mb") )
         my->chain_config->souvmoc_config.cache_size = options.at( "sou-vm-oc-cache-size-mb" ).as<uint64_t>() * 1024u * 1024u;
      if( options.count("sou-vm-oc-compile-threads") )
         my->chain_config->souvmoc_config.threads = options.at("sou-vm-oc-compile-threads").as<uint64_t>();
      if( options["sou-vm-oc-enable"].as<bool>() )
         my->chain_config->souvmoc_tierup = true;
#endif
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            auto id = blk->id();
            SOU_ASSERT( itr->second == id, checkpoint_exception,
...
void chain_apis::read_write::validate() const {
   SOU_ASSERT( !db.in_immutable_mode(), missing_chain_api_plugin_exception, "Not allowed, node in read-only mode" );
}
...
bool chain_plugin::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
   ...
   if( new_db_dir ) {
      ...
      SOU_ASSERT( reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory" );
      backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append( now );

      SOU_ASSERT( !fc::exists(backup_dir),
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         SOU_ASSERT( itr->blocknum == end + 1, gap_in_reversible_blocks_db,
   ...
   try {
      while( reversible_blocks.tellg() < end_pos ) {
         ...
         if( start == 0 ) {
            ...
            SOU_ASSERT( num == end + 1, gap_in_reversible_blocks_db,
...
void chain_plugin::handle_guard_exception(const chain::guard_exception& e) {
   ...
   elog("database chain::guard_exception, quiting..."); // log string searched for in: tests/nodesou_under_min_avail_ram.py
   // quit the app
   app().quit();
}
...
void chain_plugin::handle_db_exhaustion() {
   ...
   //return 1 -- it's what programs/nodesou/main.cpp considers "BAD_ALLOC"
   std::_Exit(1);
}

void chain_plugin::handle_bad_alloc() {
   elog("std::bad_alloc - memory exhausted");
   //return -2 -- it's what programs/nodesou/main.cpp reports for std::exception
   std::_Exit(-2);
}
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   SOU_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         SOU_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,SOU
      try {
         auto symb = sou::chain::symbol::from_string(str);
         return symb.value();
      } catch( ... ) { }
   }

   try {
      return ( sou::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
   } catch( ... ) {
      SOU_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
double convert_to_type(const string& str, const string& desc) {
   ...
   SOU_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
string convert_to_string(const chain::key256_t& source, const string& key_type, const string& encode_type, const string& desc) {
   try {
      ...
      SOU_ASSERT( false, chain_type_exception, "Incompatible key_type and encode_type for key256_t next_key" );
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   SOU_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   SOU_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = sou::chain_apis::get_abi( db, p.code );
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      SOU_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi);
      }
      SOU_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      SOU_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      SOU_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      SOU_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      SOU_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( sou::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, name(scope), N(stat), [&](const key_value_object& obj){
      SOU_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_ms, bool shorten_abi_errors ) {
   ...
   SOU_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));
   ...
   SOU_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   SOU_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const try {
   const abi_def abi = sou::chain_apis::get_abi(db, config::system_account_name);
   const auto table_type = get_table_type(abi, N(producers));
   const abi_serializer abis{ abi, abi_serializer_max_time };
   SOU_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   SOU_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  SOU_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   SOU_ASSERT( !params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64,
   ...
   if( block_num.valid() ) {
      ...
      } SOU_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))
   }

   SOU_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   SOU_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   try {
      ...
      } SOU_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      SOU_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_code_results read_only::get_code( const get_code_params& params )const {
   ...
   SOU_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            SOU_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(sou.token);
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   SOU_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer_max_time );
      auto action_type = abis.get_action_type(params.action);
      SOU_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.binargs = abis.variant_to_binary( action_type, params.args, abi_serializer_max_time, shorten_abi_errors );
      } SOU_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      SOU_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } SOU_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   // The following code makes assumptions about the contract deployed on sou account (i.e. the system contract) and how it stores its data.
   const auto& d = db.db();
   const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( N(sou), N(sou), N(rammarket) ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
      auto it = idx.find(boost::make_tuple( t_id->id, sou::chain::string_to_symbol_c(4,"RAMCORE") ));
      if( it != idx.end() ) {
...
FC_REFLECT( sou::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4) )
