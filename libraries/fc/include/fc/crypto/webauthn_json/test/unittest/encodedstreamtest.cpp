...
protected:
    ...
    void TestEncodedOutputStream(const char* expectedFilename, bool putBOM) {
        // Test FileWriteStream
        {
            ...
            EncodedOutputStream<FileEncoding, FileWriteStream> sou(os, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, sou);
                EXPECT_TRUE(success);
            }
            sou.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            EncodedOutputStream<FileEncoding, MemoryBuffer> sou(mb, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, sou);
                EXPECT_TRUE(success);
    ...
    void TestAutoUTFOutputStream(UTFType type, bool putBOM, const char *expectedFilename) {
        // Test FileWriteStream
        {
            ...
            char buffer[16];
            FileWriteStream os(fp, buffer, sizeof(buffer));
            AutoUTFOutputStream<unsigned, FileWriteStream> sou(os, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, sou);
                EXPECT_TRUE(success);
            }
            sou.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            AutoUTFOutputStream<unsigned, MemoryBuffer> sou(mb, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, sou);
                EXPECT_TRUE(success);
