...
int main(int, char*[]) {
    ...
    OutputStream sou(os, eis.GetType(), eis.HasBOM());                      // NEW
    PrettyWriter<OutputStream, UTF8<>, AutoUTF<unsigned> > writer(sou);     // CHANGED
#else
    ...
    OutputStream sou(os, true);                                             // NEW
    PrettyWriter<OutputStream, UTF8<>, UTF16LE<> > writer(sou);             // CHANGED
#endif
