...
#include <sou/vm/host_function.hpp>
#include <sou/vm/opcodes.hpp>
#include <sou/vm/signals.hpp>
    ...
    public:
      ...
      inline void reset() {
         ...
         if (_mod.memories.size()) {
            ...
            SOU_VM_ASSERT(err != -1, wasm_bad_alloc, "Cannot allocate initial linear memory.");
         }
         ...
         for (uint32_t i = 0; i < _mod.data.size(); i++) {
            ...
            SOU_VM_ASSERT(required_memory <= available_memory, wasm_memory_exception, "data out of range");
   ...
   class execution_context : public execution_context_base<execution_context<Host>, Host> {
    ...
    public:
      ...
      inline operand_stack_elem  get_global(uint32_t index) {
         SOU_VM_ASSERT(index < _mod.globals.size(), wasm_interpreter_exception, "global index out of range");
      ...
      inline void set_global(uint32_t index, const operand_stack_elem& el) {
         ...
         SOU_VM_ASSERT(gl.type.mutability, wasm_interpreter_exception, "global is not mutable");
         visit(overloaded{ [&](const i32_const_t& i) {
                                  SOU_VM_ASSERT(gl.type.content_type == types::i32, wasm_interpreter_exception,
                                                "expected i32 global type");
                                  gl.current.value.i32 = i.data.ui;
                               },
                                [&](const i64_const_t& i) {
                                   SOU_VM_ASSERT(gl.type.content_type == types::i64, wasm_interpreter_exception,
                                                 "expected i64 global type");
                                   gl.current.value.i64 = i.data.ui;
                                },
                                [&](const f32_const_t& f) {
                                   SOU_VM_ASSERT(gl.type.content_type == types::f32, wasm_interpreter_exception,
                                                 "expected f32 global type");
                                   gl.current.value.f32 = f.data.ui;
                                },
                                [&](const f64_const_t& f) {
                                   SOU_VM_ASSERT(gl.type.content_type == types::f64, wasm_interpreter_exception,
      ...
      inline void type_check(const func_type& ft) {
         for (uint32_t i = 0; i < ft.param_types.size(); i++) {
            ...
            visit(overloaded{ [&](const i32_const_t&) {
                                     SOU_VM_ASSERT(ft.param_types[i] == types::i32, wasm_interpreter_exception,
                                                   "function param type mismatch");
                                  },
                                   [&](const f32_const_t&) {
                                      SOU_VM_ASSERT(ft.param_types[i] == types::f32, wasm_interpreter_exception,
                                                    "function param type mismatch");
                                   },
                                   [&](const i64_const_t&) {
                                      SOU_VM_ASSERT(ft.param_types[i] == types::i64, wasm_interpreter_exception,
                                                    "function param type mismatch");
                                   },
                                   [&](const f64_const_t&) {
                                      SOU_VM_ASSERT(ft.param_types[i] == types::f64, wasm_interpreter_exception,
                                                    "function param type mismatch");
                                   },
      ...
      inline std::optional<operand_stack_elem> execute(Host* host, Visitor&& visitor, uint32_t func_index, Args... args) {
         SOU_VM_ASSERT(func_index < std::numeric_limits<uint32_t>::max(), wasm_interpreter_exception,
...
#define CREATE_LABEL(NAME, CODE)                                                                                  \
      ev_label_##NAME : visitor(ev_variant->template get<sou::vm::SOU_VM_OPCODE_T(NAME)>());                    \
...
#define CREATE_EMPTY_LABEL(NAME, CODE) ev_label_##NAME :  \
      ...
      void execute(Visitor&& visitor) {
         static void* dispatch_table[] = {
            SOU_VM_CONTROL_FLOW_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_BR_TABLE_OP(CREATE_TABLE_ENTRY)
            SOU_VM_RETURN_OP(CREATE_TABLE_ENTRY)
            SOU_VM_CALL_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_CALL_IMM_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_PARAMETRIC_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_VARIABLE_ACCESS_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_MEMORY_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_I32_CONSTANT_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_I64_CONSTANT_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_F32_CONSTANT_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_F64_CONSTANT_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_COMPARISON_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_NUMERIC_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_CONVERSION_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_EXIT_OP(CREATE_TABLE_ENTRY)
            SOU_VM_EMPTY_OPS(CREATE_TABLE_ENTRY)
            SOU_VM_ERROR_OPS(CREATE_TABLE_ENTRY)
            &&__ev_last
         };
         auto* ev_variant = _state.pc;
         goto *dispatch_table[ev_variant->index()];
         while (1) {
             SOU_VM_CONTROL_FLOW_OPS(CREATE_LABEL);
             SOU_VM_BR_TABLE_OP(CREATE_LABEL);
             SOU_VM_RETURN_OP(CREATE_LABEL);
             SOU_VM_CALL_OPS(CREATE_LABEL);
             SOU_VM_CALL_IMM_OPS(CREATE_LABEL);
             SOU_VM_PARAMETRIC_OPS(CREATE_LABEL);
             SOU_VM_VARIABLE_ACCESS_OPS(CREATE_LABEL);
             SOU_VM_MEMORY_OPS(CREATE_LABEL);
             SOU_VM_I32_CONSTANT_OPS(CREATE_LABEL);
             SOU_VM_I64_CONSTANT_OPS(CREATE_LABEL);
             SOU_VM_F32_CONSTANT_OPS(CREATE_LABEL);
             SOU_VM_F64_CONSTANT_OPS(CREATE_LABEL);
             SOU_VM_COMPARISON_OPS(CREATE_LABEL);
             SOU_VM_NUMERIC_OPS(CREATE_LABEL);
             SOU_VM_CONVERSION_OPS(CREATE_LABEL);
             SOU_VM_EXIT_OP(CREATE_EXIT_LABEL);
             SOU_VM_EMPTY_OPS(CREATE_EMPTY_LABEL);
             SOU_VM_ERROR_OPS(CREATE_LABEL);
             __ev_last:
