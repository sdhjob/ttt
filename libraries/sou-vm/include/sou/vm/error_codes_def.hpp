...
#define SOU_VM_OPEN_NAMESPACE namespace sou::vm {
#define SOU_VM_CLOSE_NAMESPACE }

#include "error_codes_pp.hpp"

// create a category for parser error codes
GENERATE_ERROR_CATEGORY(parser_errors, "sou-vm parser errors")
GENERATE_ERROR_CATEGORY(memory_errors, "sou-vm memory errors")
GENERATE_ERROR_CATEGORY(system_errors, "sou-vm system errors")
GENERATE_ERROR_CATEGORY(auxiliary_errors, "sou-vm auxiliary errors")
...
#define _SOU_VM_ASSERT(expr, err_type)                                                                                  \
   if (!UNLIKELY(expr)) {                                                                                              \
      return err_type;                                                                                                 \
   }

// assert but invalidate the enclosing class
#define SOU_VM_ASSERT_INVALIDATE(expr, err_type)                                                                       \
