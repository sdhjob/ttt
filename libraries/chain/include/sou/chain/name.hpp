...
namespace fc {
  class variant;
  void to_variant(const sou::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, sou::chain::name& check);
} // fc
...
namespace sou::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, sou::chain::name& check);
...
#define N(X) sou::chain::string_to_name(#X)

} // sou::chain

namespace std {
   template<> struct hash<sou::chain::name> : private hash<uint64_t> {
      typedef sou::chain::name argument_type;
...
FC_REFLECT( sou::chain::name, (value) )
