...
namespace sou { namespace chain { namespace config {
...
const static name system_account_name    { N(sou) };
const static name null_account_name      { N(sou.null) };
const static name producers_account_name { N(sou.prods) };
...
const static name sou_auth_scope       { N(sou.auth) };
const static name sou_all_scope        { N(sou.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name sou_any_name  { N(sou.any) };
const static name sou_code_name { N(sou.code) };
...
const static sou::chain::wasm_interface::vm_type default_wasm_runtime = sou::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace sou::chain::config

constexpr uint64_t SOU_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / sou::chain::config::percent_100;
}

template<typename Number>
Number SOU_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + sou::chain::config::percent_100 - sou::chain::config::percent_1)  / sou::chain::config::percent_100;
}
