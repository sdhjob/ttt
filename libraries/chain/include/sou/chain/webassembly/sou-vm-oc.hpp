...
using namespace sou::chain::souvmoc;

class souvmoc_instantiated_module;

class souvmoc_runtime : public sou::chain::wasm_runtime_interface {
   public:
      souvmoc_runtime(const boost::filesystem::path data_dir, const souvmoc::config& souvmoc_config, const chainbase::database& db);
      ~souvmoc_runtime();
      ...
      friend souvmoc_instantiated_module;
      souvmoc::code_cache_sync cc;
   ...
   static native_to_wasm_t<Ret> invoke(Translated... translated) {
      try {
         if constexpr(!is_injected) {
            ...
            constexpr int depth_assertion_intrinsic_offset = OFFSET_OF_FIRST_INTRINSIC - (int)boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("souvmoc_internal.depth_assert"))).value()*8;
         ...
         *reinterpret_cast<std::exception_ptr*>(sou_vm_oc_get_exception_ptr()) = std::current_exception();
      }
...
struct intrinsic_invoker_impl<is_injected, Ret, std::tuple<array_ptr<T>, uint32_t, Inputs...>, std::tuple<Translated...>> {
   ...
   static auto translate_one(Inputs... rest, Translated... translated, I32 ptr, I32 size) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      ...
      if ( reinterpret_cast<uintptr_t>(base) % alignof(T) != 0 ) {
         std::vector<std::byte>& copy = reinterpret_cast<std::list<std::vector<std::byte>>*>(sou_vm_oc_get_bounce_buffer_list())->emplace_back(length > 0 ? length*sizeof(T) : 1);
         T* copy_ptr = (T*)&copy[0];
...
struct intrinsic_invoker_impl<is_injected, Ret, std::tuple<T &, Inputs...>, std::tuple<Translated...>> {
   ...
   template<then_type Then, typename U=T>
   static auto translate_one(Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      SOU_ASSERT((U32)ptr != 0, wasm_exception, "references cannot be created for null pointers");
   ...
   template<then_type Then, typename U=T>
   static auto translate_one(Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<!std::is_const<U>::value, Ret> {
      SOU_ASSERT((U32)ptr != 0, wasm_exception, "reference cannot be created for null pointers");
      T &base = *array_ptr_impl<T>((uint32_t)ptr, 1);
...
#define _REGISTER_SOUVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static sou::chain::souvmoc::intrinsic _INTRINSIC_NAME(__intrinsic_fn, __COUNTER__) SOUVMOC_INTRINSIC_INIT_PRIORITY (\
      MOD "." NAME,\
      sou::chain::webassembly::souvmoc::wasm_function_type_provider<WASM_SIG>::type(),\
      (void *)sou::chain::webassembly::souvmoc::intrinsic_function_invoker_wrapper<std::string_view(MOD) != "env", WASM_SIG, SIG>::type::fn<&CLS::METHOD>(),\
      ::boost::hana::index_if(sou::chain::souvmoc::intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING(MOD "." NAME))).value()\
   );\


} } } }// sou::chain::webassembly::souvmoc
