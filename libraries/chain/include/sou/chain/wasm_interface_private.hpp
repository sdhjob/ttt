...
#include <sou/chain/webassembly/wavm.hpp>
#include <sou/chain/webassembly/wabt.hpp>
#ifdef SOU_VM_OC_RUNTIME_ENABLED
#include <sou/chain/webassembly/sou-vm-oc.hpp>
#else
#define _REGISTER_SOUVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
...
using namespace fc;
using namespace sou::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
...
#ifdef SOU_VM_OC_RUNTIME_ENABLED
      struct souvmoc_tier {
         souvmoc_tier(const boost::filesystem::path& d, const souvmoc::config& c, const chainbase::database& db) : cc(d, c, db), exec(cc) {}
         souvmoc::code_cache_async cc;
         souvmoc::executor exec;
         souvmoc::memory mem;
      };
#endif

      wasm_interface_impl(wasm_interface::vm_type vm, bool souvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const souvmoc::config& souvmoc_config) : db(d), wasm_runtime_time(vm) {
         if(vm == wasm_interface::vm_type::wabt)
            runtime_interface = std::make_unique<webassembly::wabt_runtime::wabt_runtime>();
#ifdef SOU_SOU_VM_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::sou_vm)
            runtime_interface = std::make_unique<webassembly::sou_vm_runtime::sou_vm_runtime<sou::vm::interpreter>>();
#endif
#ifdef SOU_SOU_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::sou_vm_jit)
            runtime_interface = std::make_unique<webassembly::sou_vm_runtime::sou_vm_runtime<sou::vm::jit>>();
#endif
#ifdef SOU_VM_OC_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::sou_vm_oc)
            runtime_interface = std::make_unique<webassembly::souvmoc::souvmoc_runtime>(data_dir, souvmoc_config, d);
#endif
         if(!runtime_interface)
            SOU_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef SOU_VM_OC_RUNTIME_ENABLED
         if(souvmoc_tierup) {
            SOU_ASSERT(vm != wasm_interface::vm_type::sou_vm_oc, wasm_exception, "You can't use SOU VM OC as the base runtime when tier up is activated");
            souvmoc.emplace(data_dir, souvmoc_config, d);
         }
#endif
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            SOU_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            SOU_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#ifdef SOU_VM_OC_RUNTIME_ENABLED
         if(souvmoc) for(auto it = first_it; it != last_it; it++)
            souvmoc->cc.free_code(it->code_hash, it->vm_version);
#endif
         ...
         if(!it->module) {
            ...
            if (runtime_interface->inject_module(module)) {
               try {
                  ...
                  SOU_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
               } catch (const IR::ValidationException& e) {
                  SOU_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
...
#ifdef SOU_VM_OC_RUNTIME_ENABLED
      fc::optional<souvmoc_tier> souvmoc;
#endif
   };
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   ...
   _REGISTER_SOU_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)       \
   _REGISTER_SOUVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, SOU_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
