#include <sou/chain/webassembly/sou-vm-oc/executor.hpp>
#include <sou/chain/webassembly/sou-vm-oc/code_cache.hpp>
#include <sou/chain/webassembly/sou-vm-oc/memory.hpp>
#include <sou/chain/webassembly/sou-vm-oc/intrinsic_mapping.hpp>
...
#error SOU VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, SOUVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, SOUVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic SOUVMOC_INTRINSIC_INIT_PRIORITY("souvmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
  (void*)&sou_vm_oc_grow_memory,
  boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("souvmoc_internal.grow_memory"))).value()
);

//This is effectively overriding the eosio_exit intrinsic in wasm_interface
static void eosio_exit(int32_t code) {
   siglongjmp(*sou_vm_oc_get_jmp_buf(), SOUVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
...
static void throw_internal_exception(const char* const s) {
   *reinterpret_cast<std::exception_ptr*>(sou_vm_oc_get_exception_ptr()) = std::make_exception_ptr(wasm_execution_error(FC_LOG_MESSAGE(error, s)));
   siglongjmp(*sou_vm_oc_get_jmp_buf(), SOUVMOC_EXIT_EXCEPTION);
   __builtin_unreachable();
}

#define DEFINE_SOUVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function SOUVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING(#module "." #name))).value() \
   ); \
	void name()

DEFINE_SOUVMOC_TRAP_INTRINSIC(souvmoc_internal,depth_assert) {
   throw_internal_exception("Exceeded call depth maximum");
}

DEFINE_SOUVMOC_TRAP_INTRINSIC(souvmoc_internal,div0_or_overflow) {
   throw_internal_exception("Division by 0 or integer overflow trapped");
}

DEFINE_SOUVMOC_TRAP_INTRINSIC(souvmoc_internal,indirect_call_mismatch) {
   throw_internal_exception("Indirect call function type mismatch");
}

DEFINE_SOUVMOC_TRAP_INTRINSIC(souvmoc_internal,indirect_call_oob) {
   throw_internal_exception("Indirect call index out of bounds");
}

DEFINE_SOUVMOC_TRAP_INTRINSIC(souvmoc_internal,unreachable) {
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. SOU VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, const memory& mem, apply_context& context) {
   ...
   cb->current_call_depth_remaining = sou::chain::wasm_constraints::maximum_call_depth+2;
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      ...
      case SOUVMOC_EXIT_CHECKTIME_FAIL:
         context.trx_context.checktime();
         break;
      case SOUVMOC_EXIT_SEGV:
         SOU_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case SOUVMOC_EXIT_EXCEPTION: //exception
