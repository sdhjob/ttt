...
#define SOUVMOC_MEMORY_PTR_cb_ptr GS_PTR struct sou_vm_oc_control_block* const cb_ptr = ((GS_PTR struct sou_vm_oc_control_block* const)(SOU_VM_OC_CONTROL_BLOCK_OFFSET));

int32_t sou_vm_oc_grow_memory(int32_t grow, int32_t max) {
   SOUVMOC_MEMORY_PTR_cb_ptr;
   ...
   current_gs += grow_amount * SOU_VM_OC_MEMORY_STRIDE;
...
sigjmp_buf* sou_vm_oc_get_jmp_buf() {
   SOUVMOC_MEMORY_PTR_cb_ptr;
   return cb_ptr->jmp;
}

void* sou_vm_oc_get_exception_ptr() {
   SOUVMOC_MEMORY_PTR_cb_ptr;
   return cb_ptr->eptr;
}

void* sou_vm_oc_get_bounce_buffer_list() {
   SOUVMOC_MEMORY_PTR_cb_ptr;
