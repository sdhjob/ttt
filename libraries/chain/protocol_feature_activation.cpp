...
namespace sou { namespace chain {
   ...
   void protocol_feature_activation::reflector_init() {
      ...
      SOU_ASSERT( protocol_features.size() > 0, ill_formed_protocol_feature_activation,
      ...
      for( const auto& d : protocol_features ) {
         auto res = s.insert( d );
         SOU_ASSERT( res.second, ill_formed_protocol_feature_activation,
   ...
   )
   {
      ...
      SOU_ASSERT( !enforce_disjoint || protocol_features.size() == expected_size,
