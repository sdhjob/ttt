...
#include <sou/chain/producer_schedule.hpp>
#include <sou/chain/exceptions.hpp>
#include <boost/core/ignore_unused.hpp>
#include <sou/chain/authorization_manager.hpp>
#include <sou/chain/resource_limits.hpp>
#include <sou/chain/wasm_interface_private.hpp>
#include <sou/chain/wasm_sou_validation.hpp>
...
namespace sou { namespace chain {
   ...
   wasm_interface::wasm_interface(vm_type vm, bool souvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const souvmoc::config& souvmoc_config)
     : my( new wasm_interface_impl(vm, souvmoc_tierup, d, data_dir, souvmoc_config) ) {}
   ...
   void wasm_interface::validate(const controller& control, const bytes& code) {
      ...
      try {
         ...
         SOU_ASSERT(false, wasm_serialization_error, e.message.c_str());
   ...
   void wasm_interface::apply( const digest_type& code_hash, const uint8_t& vm_type, const uint8_t& vm_version, apply_context& context ) {
      ...
      if(my->souvmoc) {
         const chain::souvmoc::code_descriptor* cd = nullptr;
         try {
            cd = my->souvmoc->cc.get_descriptor_for_code(code_hash, vm_version);
         }
         catch(...) {
            //swallow errors here, if SOU VM OC has gone in to the weeds we shouldn't bail: continue to try and run baseline
            //In the future, consider moving bits of SOU VM that can fire exceptions and such out of this call path
            static bool once_is_enough;
            if(!once_is_enough)
               elog("SOU VM OC has encountered an unexpected failure");
            once_is_enough = true;
         }
         if(cd) {
            my->souvmoc->exec.execute(*cd, my->souvmoc->mem, context);
            return;
         }
      }
#endif
   ...
   public:
      ...
      :context(ctx)
      {
         if( context.is_context_free() )
            SOU_ASSERT( context_free, unaccessible_api, "only context free api's can be used in this context" );
      }
...
class context_free_api : public context_aware_api {
   public:
      ...
      :context_aware_api(ctx, true) {
         ...
         SOU_ASSERT( context.is_context_free(), unaccessible_api, "this API may only be called from context_free apply" );
      }
...
class privileged_api : public context_aware_api {
   public:
      ...
      {
         SOU_ASSERT( context.is_privileged(), unaccessible_api, "${code} does not have permission to call this API", ("code",context.get_receiver()) );
      }
      ...
      void activate_feature( int64_t feature_name ) {
         SOU_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
      }
      ...
      void set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
         SOU_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
         SOU_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
         SOU_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
      ...
      int64_t set_proposed_producers_common( vector<producer_authority> && producers, bool validate_keys ) {
         SOU_ASSERT(producers.size() <= config::max_producers, wasm_execution_error, "Producer schedule exceeds the maximum producer count for this chain");
         SOU_ASSERT( producers.size() > 0
         ...
         for (const auto& p: producers) {
            SOU_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
            ...
            p.authority.visit([&p, num_supported_key_types, validate_keys](const auto& a) {
               ...
               for (const auto& kw: a.keys ) {
                  SOU_ASSERT( kw.key.which() < num_supported_key_types, unactivated_key_type,
                              "Unactivated key type used in proposed producer schedule");

                  if( validate_keys ) {
                     SOU_ASSERT( kw.key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
                  }
               ...
               SOU_ASSERT( a.keys.size() == unique_keys.size(), wasm_execution_error, "producer schedule includes a duplicated key for ${account}", ("account", p.producer_name));
               SOU_ASSERT( a.threshold > 0, wasm_execution_error, "producer schedule includes an authority with a threshold of 0 for ${account}", ("account", p.producer_name));
               SOU_ASSERT( sum_weights >= a.threshold, wasm_execution_error, "producer schedule includes an unsatisfiable authority for ${account}", ("account", p.producer_name));
            });


            unique_producers.insert(p.producer_name);
         }
         SOU_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
      ...
      int64_t set_proposed_producers_ex( uint64_t packed_producer_format, array_ptr<char> packed_producer_schedule, uint32_t datalen ) {
         if (packed_producer_format == 0) {
            ...
            SOU_THROW(wasm_execution_error, "Producer schedule is in an unknown format!");
         }
      }
...
#pragma GCC diagnostic pop
      ...
      int32_t _eosio_f32_trunc_i32s( float af ) {
         ...
         if (_eosio_f32_ge(af, 2147483648.0f) || _eosio_f32_lt(af, -2147483648.0f))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f32.convert_s/i32 overflow" );

         if (is_nan(a))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f32.convert_s/i32 unrepresentable");
      ...
      int32_t _eosio_f64_trunc_i32s( double af ) {
         ...
         if (_eosio_f64_ge(af, 2147483648.0) || _eosio_f64_lt(af, -2147483648.0))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f64.convert_s/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f64.convert_s/i32 unrepresentable");
      ...
      uint32_t _eosio_f32_trunc_i32u( float af ) {
         ...
         if (_eosio_f32_ge(af, 4294967296.0f) || _eosio_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f32.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f32.convert_u/i32 unrepresentable");
      ...
      uint32_t _eosio_f64_trunc_i32u( double af ) {
         ...
         if (_eosio_f64_ge(af, 4294967296.0) || _eosio_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f64.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f64.convert_u/i32 unrepresentable");
      ...
      int64_t _eosio_f32_trunc_i64s( float af ) {
         ...
         if (_eosio_f32_ge(af, 9223372036854775808.0f) || _eosio_f32_lt(af, -9223372036854775808.0f))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f32.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f32.convert_s/i64 unrepresentable");
      ...
      int64_t _eosio_f64_trunc_i64s( double af ) {
         ...
         if (_eosio_f64_ge(af, 9223372036854775808.0) || _eosio_f64_lt(af, -9223372036854775808.0))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f64.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f64.convert_s/i64 unrepresentable");
      ...
      uint64_t _eosio_f32_trunc_i64u( float af ) {
         ...
         if (_eosio_f32_ge(af, 18446744073709551616.0f) || _eosio_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f32.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f32.convert_u/i64 unrepresentable");
      ...
      uint64_t _eosio_f64_trunc_i64u( double af ) {
         ...
         if (_eosio_f64_ge(af, 18446744073709551616.0) || _eosio_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f64.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( sou::chain::wasm_execution_error, "Error, f64.convert_u/i64 unrepresentable");
...
class crypto_api : public context_aware_api {
   public:
      ...
      void assert_recover_key( const fc::sha256& digest,
         ...
         SOU_ASSERT(s.which() < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_signature_type,
           "Unactivated signature type used during assert_recover_key");
         SOU_ASSERT(p.which() < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_key_type,
           "Unactivated key type used when creating assert_recover_key");

         if(context.control.is_producing_block())
            SOU_ASSERT(s.variable_size() <= context.control.configured_subjective_signature_length_limit(),
                       sig_variable_size_limit_exception, "signature variable length component size greater than subjective maximum");

         auto check = fc::crypto::public_key( s, digest, false );
         SOU_ASSERT( check == p, crypto_api_exception, "Error expected key different than recovered key" );
      }
      ...
      int recover_key( const fc::sha256& digest,
         ...
         if (s.which() >= config::genesis_num_supported_key_types ) {
            SOU_ASSERT(publen >= 33, wasm_execution_error,
      ...
      template<class Encoder> auto encode(char* data, uint32_t datalen) {
         Encoder e;
         const size_t bs = sou::chain::config::hashing_checktime_block_size;
         while ( datalen > bs ) {
            e.write( data, bs );
            data += bs;
            datalen -= bs;
      ...
      void assert_sha256(array_ptr<char> data, uint32_t datalen, const fc::sha256& hash_val) {
         ...
         SOU_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }
...
class permission_api : public context_aware_api {
   public:
      ...
      {
         SOU_ASSERT( delay_us <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
      ...
      int64_t get_account_creation_time( account_name account ) {
         ...
         SOU_ASSERT( acct != nullptr, action_validate_exception,
...
public:
   ...
   void abort() {
      SOU_ASSERT( false, abort_called, "abort() called");
   }
   ...
   void eosio_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         ...
         SOU_THROW( eosio_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE& secondary ) {\
         SOU_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
         return context.IDX.store( scope, table, account_name(payer), id, secondary );\
      }\
      void db_##IDX##_update( int iterator, uint64_t payer, const TYPE& secondary ) {\
         SOU_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
...
class memory_api : public context_aware_api {
   public:
      ...
      char* memcpy( array_ptr<char> dest, array_ptr<const char> src, uint32_t length) {
         SOU_ASSERT((size_t)(std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) >= length,
...
class transaction_api : public context_aware_api {
   public:
      ...
      void send_inline( array_ptr<char> data, uint32_t data_len ) {
         ...
         SOU_ASSERT( data_len < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
...
class compiler_builtins : public context_aware_api {
   public:
      ...
      void __divti3(__int128& ret, uint64_t la, uint64_t ha, uint64_t lb, uint64_t hb) {
         ...
         SOU_ASSERT(rhs != 0, arithmetic_exception, "divide by zero");
...
/*
 * This api will be removed with fix for `sou #2561`
 */
...
std::istream& operator>>(std::istream& in, wasm_interface::vm_type& runtime) {
   ...
   in >> s;
   if (s == "wabt")
      runtime = sou::chain::wasm_interface::vm_type::wabt;
   else if (s == "sou-vm")
      runtime = sou::chain::wasm_interface::vm_type::sou_vm;
   else if (s == "sou-vm-jit")
      runtime = sou::chain::wasm_interface::vm_type::sou_vm_jit;
   else if (s == "sou-vm-oc")
      runtime = sou::chain::wasm_interface::vm_type::sou_vm_oc;
   else
