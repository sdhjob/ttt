#!/bin/bash
#
# sou-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# sou-tn_bounce.sh and sou-tn_roll.sh scripts.

connected="0"

rundir=programs/nodesou
prog=nodesou

# Quote any args that are "*", so they are not expanded
qargs=`echo "$*" | sed -e 's/ \* / "*" /' -e 's/ \*$/ "*"/'`

if [ "$PWD" != "$SOU_HOME" ]; then
    echo $0 must only be run from $SOU_HOME
    exit -1
fi
...
datadir=var/lib/node_$SOU_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/sou/node_$SOU_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/sou/node_$SOU_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
