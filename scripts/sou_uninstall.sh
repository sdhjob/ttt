...
function usage() {
  ...
  -i DIR      Directory where sou is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the sou data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export SOU_INSTALL_DIR=${INSTALL_LOCATION:-$SOU_INSTALL_DIR}

if [[ ! -d "${SOU_INSTALL_DIR}" ]]; then
   echo "[SOU installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${SOU_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/sou/sou/issues/6940
   echo "[SOU installation found: ${SOU_INSTALL_DIR}]" && INSTALL_PATHS+=("${SOU_INSTALL_DIR}") # SOU_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${SOU_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/sou")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/sou")
            fi
            # Version < 1.8.0; Before we started using ~/sou/1.8.x
            ...
            echo " - SOU Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled SOU Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing SOU Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/sou_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
